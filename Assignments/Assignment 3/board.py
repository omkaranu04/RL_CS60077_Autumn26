...
# Cell values and display values
EMPTY, X, O = 0, 1, 2
...
class Board:
    """NxN tic-tac-toe environment: board state, move, reward"""
    def __init__(self, size=3):
        self.size = size
        self.n_cells = size * size
        self.max_steps = 10 * self.n_cells      # cap to guard against run away episodes
        self._lines = self._winning_lines()     # precomputed list of cell-index groups that constitute a win
        self.reset()

    def _winning_lines(self):
        """building all the winning lines: every row, every column, 2 diagonals"""
        lines = []
    ...
    @property
    def n_states(self):
        """total staate space 3 ^ (n x n) -> every cell can be either empty/X/O"""
        return 3 ** self.n_cells

    @property
    def n_actions(self):
        """moves = total cells, after that exhaust"""
        return self.n_cells

    def reset(self):
        """ clear the board and return state index of empty board"""
    ...
    def state_idx(self, state):
        """encode a cell tuple as a single integer"""
        idx = 0
        for cell in state:
            idx = idx * 3 + cell
        return idx
    ...
    def state_from_idx(self, idx):
        """inverse of what is done in state_idx"""
    ...
    def check(self, action):
        """checking the bounds of the action number"""
        return 0 <= action < self.n_cells

    def available_actions(self, state=None):
        """list of the empty cells for the given state"""
        cells = state if state is not None else self.cells
        return [i for i, v in enumerate(cells) if v == EMPTY]

    def winner(self, cells=None):
        """return the winning player's mark, or None if no line is fully owned by any player"""
    ...
    def step(self, action):
        ...
        if not self.check(action) or self.cells[action] != EMPTY:
            # invalid move -> same board, player has to retry
        ...
        if not done:
            self.current_player = O if player == X else X   # pass the turn on a valid non-terminal move
